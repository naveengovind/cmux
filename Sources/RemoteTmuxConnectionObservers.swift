...
final class RemoteTmuxConnectionObservers {
    ...
    ///   - onSessionsChanged: fires on `%sessions-changed` — the server-wide
    ///     session SET changed (a session was created or destroyed anywhere on
    ///     the server, not necessarily the one this connection is attached to).
    ///     Consumers reconcile the mirrored-session set against fresh discovery.
    ...
    /// (`%sessions-changed`: a session was created or destroyed on the server).
